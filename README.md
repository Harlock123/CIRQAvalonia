...
The palette has a **Find a part** box (`Ctrl+F`): 183 components in 16 groups is more than anybody
...
shot noise, MOSFET channels bring their own; flicker noise and op-amp input noise are not modelled
and the docs say so rather than letting the figure look complete.
...
| Noise examples | The Inverting Amplifier's noise being entirely its resistors, a hundred times the resistance being ten times the noise, the RC Low-Pass refusing to get quieter across a hundredfold change of resistor — and the window opening on the output probe rather than an input a source holds at no noise at all |
