...
`baseline` and `compare` are the other half of the same idea: record what the circuit does now, and
fail a build later when anything moves by more than a percent. Requirements catch "it stopped meeting
spec"; a baseline catches "you changed something you did not mean to", which is the failure that
...
The requirements can be held against the circuit **across a range** rather than at one point, which
is what a design is actually signed off against: every enabled requirement, run at each of a series
of temperatures — or with any part stepped — and reported as the range it holds over, the worst
point, and what it read there. What is plotted is the *margin*, because that is the one thing every
requirement has in common whatever it measures: a fraction of its own limit, with zero the line none
may cross. The verdicts come from the same code that produces them in the requirements window, so
the two cannot disagree about whether the circuit passes.
...
**A block can be given a symbol.** *Edit > Draw the Block's Symbol* sets its size, puts each pin on
whichever edge you want it at whatever height, and names the pins the way a datasheet would —
`IN+`, `OUT`, `GND` — with the real block drawn by the real canvas beside the boxes as you type.
Every block was the same grey box with its pins alternating down the sides, which is fine for hiding
a section and useless for making a part: what says *amplifier* rather than *regulator* at a glance is
the shape and where the pins are. It is drawing and only drawing — a pin that moves or is renamed is
the same pin, so the wires on it stay on it and the circuit solves to the same answer — and the
symbol is saved with the circuit and travels into the block library, which is what turns that from a
drawer of grey boxes into a parts library. Drawing the guide's figure for it also turned up a bug as
old as blocks: a pin was placed against the block's height *at the moment it was added*, and the
block grows with each pin, so a five-pin block drew two of them on top of each other.
...
main scope and is saved with the file. That was the real gap: a block's contents solve as ordinary
parts, so a node inside one always had a voltage, and the canvas only draws what is on the sheet, so
there was never anything to click. A hierarchy you cannot measure inside is half a hierarchy.
...
**Tolerance analysis runs on every core.** The trials are independent circuits, which made it the
one embarrassingly parallel workload here and a serial loop that left eleven cores of twelve
watching. What made it serial was the circuit rather than the arithmetic — a trial writes values
into the parts — so each worker gets its own copy, made through the serializer because a copy that
is not the saved form is a second definition of what a circuit is. Measured on a twenty-part circuit
at two thousand trials: 262 ms to 71 ms on eight cores. The answer is identical reading for reading,
because each trial's parts come from its own number rather than from a shared stream — a result that
changed with the number of cores would not be a result.
...
| Worst case over temperature | A diode reference with no toleranced part in it being immovable until a range is asked for, the temperature then being a corner axis with the cold end giving the high corner, parts and temperature varied together giving a wider spread than either alone and both named in the recipe, a range of no width not counting as an axis, and the document's own ambient left exactly as it was found |
| Requirements over a range | A diode reference checked from −40 °C to +85 °C: met at the cold end and not at the hot one, the worst case landing on the end of the range that fails, the range it does hold over named, a requirement with a comfortable limit holding throughout, one about a trace that is not there coming back without a verdict rather than as a failure, the same sweep run over a part's value instead of the temperature, and the circuit — temperature, part values and operating point — put back exactly as it was afterwards |
...
| Block symbols | A symbol starting from whatever the block looks like now, size and pins reaching the block as they are typed, a pin going onto whichever edge it is put on and staying on the body however far along it is pushed, arranging putting them back and leaving no symbol of its own, a drawn symbol surviving a save while an undrawn one writes nothing about itself — and the one that matters: moving and renaming pins leaving every wire in place and the circuit solving to the same answer to nine figures |
...
An **export** offers every page — a page each in a PDF, a file each named after the sheet in the
picture formats — and a **print** always gives every page, because a printed schematic missing two
of its three sheets is not a printed schematic.

A label whose net reaches another page **says so under its name** — `→ Logic` — worked out from the
drawing rather than typed, so it cannot go stale the way a hand-placed off-page connector can, and a
rail you expected to see it under and do not is a typo you have just found. Double-clicking the
label follows the net there and selects the one at the far end.
...
**KiCad netlist** for a board, the traces as **CSV**, and the parts list as a **BOM** — quantity,
designators, part, value and footprint, with two otherwise identical parts in different packages
counted as two lines, because they are two things to order.
...
| Print | `Ctrl+P` gives every sheet a page, with a **title block** across the top: what the drawing is, which sheet, the revision, who drew it, the date and the page number. Rev and Drawn by are typed in the print dialog and belong to the circuit, not to that print. It prints in ink whatever theme the screen is wearing |
