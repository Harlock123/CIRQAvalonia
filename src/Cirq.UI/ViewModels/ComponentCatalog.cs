    ...
    [
        ...
        [
            ...
            new("Crystal", "Quartz resonator — the frequency comes from the part, not the circuit",
                () => new Crystal()),
            new("Ferrite Bead", "Lossy above a few megahertz, a wire below it — not an inductor",
                () => new FerriteBead()),
            new("Common-Mode Choke", "Passes the signal and stops the noise — it selects by direction, not frequency",
                () => new CommonModeChoke()),
            new("Transmission Line", "A metre of 50 Ω coax, with the delay and the reflections that come with it",
        ...
        [
            ...
            new("VCVS", "Voltage-controlled voltage source — SPICE's E element, and the primitive every amplifier is made of",
        ...
        [
            ...
            new("Varistor (MOV)", "Mains surge absorber — soft knee, and it wears out",
                () => new Varistor()),
            new("Photodiode", "Light straight to current — no gain, and none of the delay gain costs",
                () => new Photodiode()),
            new("Varactor", "A capacitance you tune with a voltage — how a radio was tuned",
                () => new Varactor()),
        ...
        [
            ...
            new("IGBT", "A MOSFET gate on a bipolar output — a voltage drop, not a resistance",
                () => new Igbt()),
        ...
        [
            ...
            new("LM358", "Single-supply op-amp, gets closer to the rails than a 741",
                () => new OperationalAmplifier(OpAmpModel.Lm358)),
            new("MCP6002", "Rail-to-rail CMOS op-amp — compare its swing with the LM358's",
                () => new OperationalAmplifier(OpAmpModel.Mcp6002)),
            new("LM324 Quad Op-Amp", "Four LM358s in one 14-pin package, sharing one supply pair",
                () => new QuadOpAmp()),
            new("Analog Multiplier", "AD633 — W = XY/10 + Z, which is modulation, mixing and RMS",
                () => new AnalogMultiplier()),
        ...
        [
            ...
            new("74161", "Synchronous binary counter — every output changes on the same edge",
                () => new Ic74161()),
            new("74373", "Octal transparent latch — a wire while its enable is high, a memory after",
                () => new Ic74373()),
            new("74245", "Octal bus transceiver — the only tri-state outputs in the palette",
                () => new Ic74245()),
        ...
        [
            ...
            new("4046", "Phase-locked loop — a VCO, two comparators, and the filter is yours to add",
                () => new Ic4046()),
        ...
        [
            ...
            new("Serial Device", "UART module that greets you and echoes back, at its own baud rate",
                () => new SerialDevice()),
            new("I2C ADC", "ADS1115 — reads an analog node and reports it over two wires",
                () => new Ads1115()),
            new("I2C DAC", "MCP4725 — the other way round, and full scale is whatever the supply is",
                () => new Mcp4725()),
            new("I2C Current Sensor", "INA219 — a shunt in the high side, and a common-mode limit to respect",
                () => new Ina219()),
            new("1-Wire Master", "Plays a written list of 1-Wire operations down a single wire",
                () => new OneWireMaster()),
            new("1-Wire Thermometer", "DS18B20 — degrees over one wire. Double-click to warm it",
                () => new Ds18b20()),
            new("SPI ADC", "MCP3008: eight inputs, ten bits, one transaction — the answer arrives while you ask",
                () => new Mcp3008()),
            new("CAN Transceiver", "Wired-AND differential bus — a dominant bit beats a recessive one",
                () => new CanTransceiver()),
            new("RS-485 Transceiver", "Differential signalling down a long cable — and an idle bus that floats",
                () => new Rs485Transceiver()),
            new("RS-232 Transceiver", "MAX232 — makes its own ±8.5 V from the 5 V rail, and inverts both ways",
                () => new Max232()),
        ...
        [
            ...
            new("SRAM 2Kx8", "Addressed memory with tri-state data — type its contents in, read them back",
                () => new MemoryDevice()),
            new("ROM 2Kx8", "The same part with writes refused, for holding a program rather than a variable",
                () => new MemoryDevice { IsReadOnly = true }),
        ...
        [
            ...
            new("Stepper Motor", "Four-phase unipolar, the ULN2003's usual passenger",
                () => new StepperMotor()),
            new("Stepper Motor (Bipolar)", "The same iron with no common wire — two windings, and it needs an H-bridge each",
                () => new StepperMotor { Wiring = StepperWiring.Bipolar, CoilResistance = 2.0, CoilInductance = 2e-3 }),
            new("Ultrasonic Ranger", "HC-SR04 — the distance is in the width of the echo pulse",
                () => new UltrasonicRanger()),
            new("Hall Sensor", "Open-collector magnetic switch with hysteresis. Double-click to bring a magnet up",
                () => new HallSensor()),
            new("Phototransistor", "Light in, current out — fast, unlike the LDR. Double-click to shade it",
                () => new Phototransistor()),
            new("Reed Switch", "A contact a magnet closes. No supply, either pole, and it bounces",
                () => new ReedSwitch()),
            new("PIR Sensor", "Movement, not presence — and it holds its output long after you stop",
                () => new PirSensor()),
        ...
        [
            new("Gate Driver", "Amps into a MOSFET gate, at its own supply rather than the logic rail",
                () => new GateDriver()),
            ...
            new("Resettable Fuse", "PPTC — it does not open, it heats until it is nearly open, and it comes back",
                () => new ResettableFuse(0.5)),
            ...
            new("ULN2003", "Seven Darlington sinks — the way logic drives relays and steppers",
                () => new Uln2003()),
            new("Stepper Driver A4988", "Step and direction in, two chopped coil currents out — and it microsteps",
                () => new StepperDriver()),
            new("Solid State Relay", "Opto-isolated triac that waits for the mains to cross zero before it fires",
                () => new SolidStateRelay()),
